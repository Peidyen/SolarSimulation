...
namespace SolarSim.Physics
{
    public class NBody
    {
        ...
        public double Time { get; private set; } // seconds

        public NBody(IEnumerable<Body> bodies)
        {
        ...
        private Vector3d AccelerationOn(int i, IReadOnlyList<Body> state)
        {
            ...
            for (int j = 0; j < state.Count; j++)
            {
                ...
                var dist = r.Magnitude() + 1e-3; // softening to avoid singularities
        ...
        // 4th-order Runge-Kutta step for all bodies simultaneously
        public void Step(double dt)
        {
            ...
            var yk = new Body[n];
            for (int i = 0; i < n; i++)
                yk[i] = new Body(y0[i].Name, y0[i].Mass,
                    y0[i].Position + k1_p[i] * (dt / 2.0),
                    y0[i].Velocity + k1_v[i] * (dt / 2.0));
            for (int i = 0; i < n; i++) { k2_p[i] = yk[i].Velocity; k2_v[i] = AccelerationOn(i, yk); }

            // k3 state
            for (int i = 0; i < n; i++)
                yk[i] = new Body(y0[i].Name, y0[i].Mass,
                    y0[i].Position + k2_p[i] * (dt / 2.0),
                    y0[i].Velocity + k2_v[i] * (dt / 2.0));
            for (int i = 0; i < n; i++) { k3_p[i] = yk[i].Velocity; k3_v[i] = AccelerationOn(i, yk); }

            // k4 state
            for (int i = 0; i < n; i++)
                yk[i] = new Body(y0[i].Name, y0[i].Mass,
                    y0[i].Position + k3_p[i] * dt,
                    y0[i].Velocity + k3_v[i] * dt);
        ...
        // Simple helpers
        public void ResetTo(IEnumerable<Body> bodies)
        {
            Bodies.Clear();
