...
namespace SolarSim.Physics
{
    public readonly struct DiagnosticsResult
    {
        ...
        public DiagnosticsResult(double ke, double pe, Vector3d L){ KineticEnergy = ke; PotentialEnergy = pe; AngularMomentum = L; }
    }
    ...
    public static class Diagnostics
    {
        public static DiagnosticsResult Compute(NBody sim)
        {
            ...
            var bodies = sim.Bodies; int n = bodies.Count;

            for (int i=0;i<n;i++)
            {
                var b = bodies[i]; var v = b.Velocity;
