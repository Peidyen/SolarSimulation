...
class MCPServer
{
    ...
    private readonly JsonSerializerOptions _opts = new(JsonSerializerDefaults.Web){ WriteIndented = false };
    public MCPServer(){ _sim = new NBody(InitialSystems.SunEarthMarsJupiterSaturn()); }
    public void Run()
    {
        string? line;
        while((line = Console.ReadLine()) != null)
        {
            try
            {
                ...
                {
                    case "tools/list":
                        Respond(req.Id, new{ tools = new object[] {
                            ...
                            new { name="load_preset", description="Load SunEarthMars or SunEarthMarsJupiterSaturn", schema = new { name="string" } },
                            new { name="set_body_state", description="Set a body's state", schema = new { name="string", mass="kg (double, optional)", position="[x,y,z] m", velocity="[vx,vy,vz] m/s" } },
                            new { name="add_body", description="Add a new body", schema = new { name="string", mass="kg (double)", position="[x,y,z] m", velocity="[vx,vy,vz] m/s" } }
                        }});
                        break;
                    case "tools/call":
                        HandleToolCall(req);
    ...
    private void HandleToolCall(RpcRequest req)
    {
        ...
        switch(tool)
        {
            case "list_bodies":
                ...
                break;
            case "get_state":
                Respond(req.Id, new { time_seconds = _sim.Time, bodies = _sim.Bodies.ConvertAll(Mapper.ToDTO) });
                break;
            case "diagnostics":
            {
                var d = SolarSim.Physics.Diagnostics.Compute(_sim);
                Respond(req.Id, new DiagnosticsDTO{ kinetic=d.KineticEnergy, potential=d.PotentialEnergy, total=d.TotalEnergy, L=new[]{d.AngularMomentum.X,d.AngularMomentum.Y,d.AngularMomentum.Z}, L_mag=d.AngularMomentumMagnitude });
                break;
            }
            case "step":
            ...
            case "load_preset":
            {
                ...
                {
                    case "sunearthmars":
                    case "sem": _sim.ResetTo(InitialSystems.SunEarthMars()); break;
                    case "sunearthmarsjupitersaturn":
                    case "semjs": _sim.ResetTo(InitialSystems.SunEarthMarsJupiterSaturn()); break;
                    default: RespondError(req.Id, -32005, $"Unknown preset '{name}'"); return;
                }
                Respond(req.Id, new { ok=true });
                break;
            }
            case "set_body_state":
            {
                ...
                b.Position = new Vector3d(p[0],p[1],p[2]); b.Velocity = new Vector3d(v[0],v[1],v[2]);
                Respond(req.Id, new { ok=true });
                break;
            }
            case "add_body":
    ...
    private void Respond<T>(string? id, T payload){ var res = new RpcResponse<T>{ JsonRpc="2.0", Id=id, Result=payload }; Console.WriteLine(JsonSerializer.Serialize(res, _opts)); }
    private void RespondError(string? id, int code, string message){ var res = new RpcResponse<object>{ JsonRpc="2.0", Id=id, Error=new{ code, message } }; Console.WriteLine(JsonSerializer.Serialize(res, _opts)); }
}
class Program{ static void Main(string[] args){ Console.Error.WriteLine("SolarSim MCP server listening on STDIN/STDOUT (JSON-RPC 2.0)"); new MCPServer().Run(); } }
